...
When the region is inactive, the command =symtex-dwim= behaves similarly, except that =expr= is not set to anything before the SAGE expression that we provide is evaluated.
...
* Issues
- If you haven't already launched a =*Sage*= buffer and executed some commands in that, then =symtex-dwim= typically doesn't work correctly the first time it has been called, during to some deprecation warnings.  Sorry!  Just try it again and it should work from that point forward.

- This package relies on converting objects back and forth between SymPy and Sage, which introduces issues now and then.  The SAGE member functions ~._sage()~ and ~._sympy()~ can be useful to play around with; see for instance the horrendous contents of the customization variable =symtex-sympy2latex-expr=.  
